...
global nick_registr
nick_registr = ['Пра-во', 'Банк', 'МРЭО', 'ФСБ', 'ГИБДД', 'ГУВД', 'Армия', 'ТСР', 'НГ-А', 'МЗ-А', 'МЗ-Л', 'Судья', 'ФМ', 'СТ', 'СБ', 'ЧК', 'КМ', 'РМ', 'УМ']

global gos
gos = ['Пра-во', 'Банк', 'МРЭО', 'ФСБ', 'ГИБДД', 'ГУВД', 'Армия', 'ТСР', 'НГ-А', 'МЗ-А', 'МЗ-Л', 'Судья'] 
...
async def avatar(ctx, member: discord.Member = None):
    ...
    embed = discord.Embed( description = f'''{author.mention}, вот аватарка пользователя {user.mention}:''', color= 0xFB9E14)
...
async def on_message(ctx):
    ...
    if msg.replace('-', '').replace('+', '') in role_registr:
        ...
        if not z in nick_registr:
            ...
            embed.add_field(name = f'**Тэги Государственных Организаций:**', value = f'> `Пра-во, Банк, МРЭО, ФСБ, ГИБДД, ГУВД, Армия, ТСР, НГ-А, МЗ-А, МЗ-Л, Судья`')
