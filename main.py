...
async def button_accept_r(ctx):
    ...
    message = ctx.message
    memb = ctx.member
    role_checkers = [865145094331826177, 862361473988821012, 848630088170078269, 860207024930160680, 478970291444383766, 848630088170078269, 848658294759227392, 848660081222483988, 848661195166842900, 848663860399046666, 848664593524850748, 848971714858844191, 848973072006512676, 848974272541622362, 848977375093325844, 848977843228115024, 479198132211548161, 479049028705976340, 479049200768647169, 479198415578988554, 479198488563810315, 479185785510166567]
...
async def on_message(ctx):
    ...
    nick_registr = ['Пра-во', 'РОВД', 'Банк', 'ФСБ', 'РУВД', 'ГУВД', 'Армия', 'ТСР', 'МРЭО', 'НГ-А', 'А-МВД', 'НГ-Л', 'МЗ-А', 'МЗ-Э', 'Судья', 'КМ', 'ФМ', 'СТ', 'СБ', 'РМ', 'УМ', 'ЧК'] # Сюда вообще все теги добавь, если там что-то новое будет, если заменят, удаляй то, чего больше нету и вставляй новое, так же.
    gos = ['Пра-во', 'РОВД', 'Банк', 'ФСБ', 'РУВД', 'ГУВД', 'МРЭО', 'Армия', 'ТСР', 'НГ-А', 'А-МВД', 'НГ-Л', 'МЗ-А', 'МЗ-Э', 'Судья'] # Сюда все теги госов, по такому же принципу, если вдруг новые появятся 
    ...
    if 'снять роль у' in msg:
        ...
        role_checkers = [865145094331826177, 862361473988821012, 848630088170078269, 860207024930160680, 479048866704916540, 478970291444383766, 848630088170078269, 848658294759227392, 848660081222483988, 848661195166842900, 848663860399046666, 848664593524850748, 848971714858844191, 848973072006512676, 848974272541622362, 848977375093325844, 848977843228115024, 479198132211548161, 479049028705976340, 479049200768647169, 479198415578988554, 479198488563810315, 479185785510166567]
        z = 0
