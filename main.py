...
global nick_registr
nick_registr = ['Пра-во', 'Банк', 'ЦЛ', 'ФСБ', 'ГИБДД', 'ГУВД', 'Армия', 'ТСР', 'НГ-А', 'МЗ-А', 'МЗ-Л', 'Судья', 'ФМ', 'СТ', 'СБ', 'ЧК', 'КМ', 'РМ', 'УМ']

global gos
gos = ['Пра-во', 'Банк', 'ЦЛ', 'ФСБ', 'ГИБДД', 'ГУВД', 'Армия', 'ТСР', 'НГ-А', 'МЗ-А', 'МЗ-Л', 'Судья'] 
...
async def on_message(ctx):
    ...
    if msg.replace('-', '').replace('+', '') in role_registr:
        ...
        if not z in nick_registr:
            ...
            embed.add_field(name = f'**Тэги Государственных Организаций:**', value = f'> `Пра-во, Банк, ЦЛ, ФСБ, ГИБДД, ГУВД, Армия, ТСР, НГ-А, МЗ-А, МЗ-Л, Судья`')
