...
global nick_registr
nick_registr = ['Пра-во', 'Банк', 'МРЭО', 'ФСБ', 'ГИБДД', 'ГУВД', 'РОВД', 'Армия', 'ТСР', 'НГ-А', 'МЗ-А', 'МЗ-Л', 'МЗ-Э', 'Судья', 'ФМ', 'СТ', 'СБ', 'ЧК', 'КМ', 'РМ', 'УМ']

global gos
gos = ['Пра-во', 'Банк', 'МРЭО', 'ФСБ', 'ГИБДД', 'ГУВД', 'РОВД', 'Армия', 'ТСР', 'НГ-А', 'МЗ-А', 'МЗ-Л', 'МЗ-Э', 'Судья'] 
...
async def on_message(ctx):
    ...
    if msg.replace('-', '').replace('+', '') in role_registr:
        ...
        if not z in nick_registr:
            ...
            embed.add_field(name = f'**Тэги Государственных Организаций:**', value = f'> `Пра-во, Банк, МРЭО, ФСБ, ГИБДД, ГУВД, РОВД, Армия, ТСР, НГ-А, МЗ-А, МЗ-Л, МЗ-Э, Судья`')
