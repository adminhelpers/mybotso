...
global RCH
RCH = ['Пра-во', 'ФСБ', 'ГИБДД', 'ГУВД', 'ПЭ', 'МЗ', 'ПЛ', 'Армия', 'ФСИН', 'ЦБ', 'НГ', 'РОВД', 'МРЭО', 'КМ', 'ФМ', 'СТ', 'СБ', 'РМ', 'УМ', 'ЧК']
...
role_checkers = [478970291444383766, # ★ Правительство ★
                ...
                848971714858844191, # ★ Федеральная Служба Исполнения Наказаний ★
...
global nick_registr
nick_registr = ['Пра-во', 'Банк', 'МРЭО', 'ФСБ', 'ГИБДД', 'ГУВД', 'РОВД', 'Армия', 'ФСИН', 'НГ-А', 'МЗ-А', 'МЗ-Э', 'Судья', 'ФМ', 'СТ', 'СБ', 'ЧК', 'КМ', 'РМ', 'УМ']

global gos
gos = ['Пра-во', 'Банк', 'МРЭО', 'ФСБ', 'ГИБДД', 'ГУВД', 'РОВД', 'Армия', 'ФСИН', 'НГ-А', 'МЗ-А', 'МЗ-Э', 'Судья'] 
...
ROLES = {
  ...
  'ФСИН': 848971714858844191, # ★ Федеральная Служба Исполнения Наказаний ★
...
async def on_message(ctx):
    ...
    if msg.replace('-', '').replace('+', '') in role_registr:
        ...
        if not z in nick_registr:
            ...
            embed.add_field(name = f'**Тэги Государственных Организаций:**', value = f'> `Пра-во, Банк, МРЭО, ФСБ, ГИБДД, ГУВД, РОВД, Армия, ФСИН, НГ-А, МЗ-А, МЗ-Э, Судья`')
