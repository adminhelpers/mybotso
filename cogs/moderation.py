...
def is_accept_guild(guild_id):
  if int(guild_id) in [477547500232769536]: return 1
  else: return 0
...
def is_accept(guild_id, mas, member: discord.Member):
  if guild_id == 477547500232769536: # rodina 03
    if mas == 'mo': mas = [743887697327816705, # Заместитель Гл. Модератора
          ...
          984503272767356998, # мое
          918629809461010472, # Главный Модератор
    ...
    if mas == 'mt': mas = [822340589894893578, # ⚒ Главный Модератор ⚒
          822442266844725260, # ⚒ Зам.Главного Модератора ⚒
          642319092174553091, # ム Technical Administrator Discord ム
          822435014897434624] # ⚒ Старший Модератор ⚒
    ...
    async def mute(self, ctx, member: discord.Member = None, arg = None, *, reason = None):
        ...
        if not reason == None:
          ...
          if ath[-1] == 'voice' or reason.endswith('v') or ath[0] == 'voice':
            ...
            f = 0
        if ctx.guild.id == 477547500232769536: mute_role = discord.utils.get(ctx.guild.roles, id = 800085900435652678) # rodina 03
        elif ctx.guild.id == 465086262383083520: mute_role = discord.utils.get(ctx.guild.roles, id = 708688299467997266) # rodina 02
        sleep = 0
