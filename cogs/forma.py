                ...
                if str(react.emoji) == '❌':
                    ...
                    if cmd == 'ban':
                        ...
                        if int(s) == 2:
                            ...
                            embed = discord.Embed(title = '\⛩️ **__Временный бан по форме__**', colour = 0xFB9E14, timestamp = datetime.datetime.utcnow())
                            embed.add_field(name = '**__Пользователь:__**', value = f'{member.display_name}`({member})`', inline = False)
                            embed.add_field(name = '**__Модератор отправивший форму:__**', value = f'{modr.mention}`({modr})`', inline = False)
                            embed.add_field(name = '**__Модератор принявший форму:__**', value = f'{ctx.author.mention}`({ctx.author})`', inline = False)                             
                            embed.add_field(name = '**__Время:__**', value = f'10 дней', inline = False)
                            embed.add_field(name = '**__Причина:__**', value = f'3/6 предупреждений', inline = False)
                            ...
                            embed = discord.Embed(colour = member.color, timestamp = ctx.message.created_at) 
                            embed.set_author(name = f'\⛩️ **__Пользователь был забанен по форме!__**')
                            embed.add_field(name = '**__Пользователь:__**', value = f'{member.display_name}`({member})`', inline = False) 
                            embed.add_field(name = '**__Модератор отправивший форму:__**', value = f'{modr.mention}`({modr})`', inline = False)    
                            embed.add_field(name = '**__Модератор принявший форму:__**', value = f'{ctx.author.mention}`({ctx.author})`', inline = False) 
                            embed.add_field(name = '**__Причина:__**', value = f'6/6 Предупреждений | `(Команда {prefix}warn)` | Last: {reason}', inline = False)  
                            embed.set_thumbnail(url = 'https://images-ext-1.discordapp.net/external/yarwcyEZug1mZITDcgLOQKSbDh7O6361bRAu7S95qNU/https/avatars.mds.yandex.net/get-pdb/2826470/29569d4a-36f3-4b9c-94f5-027c7cfb03f6/s1200')
                            embed.set_footer(text = f'Support Team by dollar ム baby#3603', icon_url = 'https://images-ext-1.discordapp.net/external/cVW5pAsyoLnQiTP-DZzQ3hLnIq-2Kw3rBZUVZ33Cz30/%3Fsize%3D1024/https/cdn.discordapp.com/avatars/729309765431328799/684fd7878d39ba93511700dbf7a45931.webp?width=677&height=677')
                            channel = self.bot.get_channel(834039427541631016)
                            await channel.send(embed = embed) 
                            ...
                            warns.update_one({"proverka": 1}, {"$set": {"numbed": warns.find_one({"proverka": 1})["numbed"] + 1}})
                            chan = self.bot.get_channel(834039427541631016)
                            embed = discord.Embed(title = '\⛩️ __**Пользователю выдано предупреждение по форме!__**', colour = 0xFB9E14, timestamp = ctx.message.created_at)
                            embed.add_field(name = '**__Пользователь:__**', value = f'{member.display_name}`({member})`', inline = False) 
                            embed.add_field(name = '**__Модератор отправивший форму:__**', value = f'{modr.mention}`({modr})`', inline = False)    
                            embed.add_field(name = '**__Модератор принявший форму:__**', value = f'{ctx.author.mention}`({ctx.author})`', inline = False) 
                            embed.add_field(name = '**__Количество предупреждений:__**', value = f'{s + 1}/3', inline = False)
                            embed.add_field(name = '**__Причина:__**', value = f'{reason}', inline = False) 
                            embed.set_footer(text = f'Support Team by dollar ム baby#3603', icon_url = self.bot.user.avatar_url)
                            embed.set_thumbnail(url = ctx.guild.icon_url)
