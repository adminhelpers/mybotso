...
class golosovanie(commands.Cog):
    ...
    async def moving(self, ctx, member: discord.Member):
        ...
        try:
            ...
            await ctx.send(f'{ctx.author.mention}', discord.Embed(title = '\⛩️ **__Переносчик пользователей__**', description = f'{ctx.author}, вы успешно переместили пользователя {member.name}`({member.mention})` в голосовой канал {schannel.mention}'))
